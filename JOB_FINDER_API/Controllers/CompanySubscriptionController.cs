        ...
        private async Task ProcessCompanyPayment(Payment payment)
        {
            ...
            try
            {
                ...
                if (existingSubscription != null)
                {
                    ...
                    existingSubscription.RemainingJobPosts += subscriptionType.JobPostLimit;
                    existingSubscription.UpdatedAt = DateTime.UtcNow;
                    _logger.LogInformation($"Extended company subscription for user {payment.UserId} until {existingSubscription.EndDate}, added {subscriptionType.JobPostLimit} job posts");
                }
                else
                {
                    // Create new subscription
                    ...
                    _context.CompanySubscriptions.Add(subscription);
                    _logger.LogInformation($"Created new company subscription for user {payment.UserId} until {subscription.EndDate}, with {subscription.RemainingJobPosts} job posts");
                }
            ...
            try
            {
                ...
                if (subscription == null)
                {
                    ...
                    return Ok(new
                    {
                        ...
                        Message = $"You are on the Free tier. Upgrade to post more jobs and see more CV matches."
                    });
                }
